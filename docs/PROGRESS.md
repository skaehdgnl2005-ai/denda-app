...
| **Sprint 4** | W3 | S04 + S06 + S12 + S15 (TestFlight) | **S04 ✅ DONE 2026-05-26** (backend + UI via S05-screen-confirm) / **S06 sub-task 10/N 완료 2026-05-26**: queue-foundation + worker-integration + google-oauth + migration-0011 + apple-expo-calendar + worker-google-integration + worker-apple-trigger + setup + applesync-hook + **ui-first-time-modal** (D34/D35 신규. 양 provider backend + lib + setup wiring + AppState hook + 첫 캘린더 선택 모달 완성. reauth modal + applesync 전역 wire-up 잔여) / S12·S15 미시작 | IN_PROGRESS (early start) |
...
| W3 (Sprint 4) | **S04 ✅ DONE 정식** + **S06 sub-task 10/N 완료 2026-05-26**: queue-foundation + worker-integration + google-oauth + migration-0011 + apple-expo-calendar + worker-google-integration + worker-apple-trigger + setup + applesync-hook + **ui-first-time-modal**. D34(Q-B22 close)·D35 신규 | 4 | Sprint 4 early start. S06 backend + lib + setup wiring + AppState hook + 첫 캘린더 선택 모달 완성. reauth modal + applesync 전역 wire-up 잔여 — 에뮬레이터 시각 검증 별도 세션 |
| W3.5 | — | — | Launch |
...
- **날짜**: 2026-05-26
- **업데이트한 사람**: S06-ui-first-time-modal ship. `src/components/calendar/FirstTimeModal.tsx` 신규 — 4 옵션(Google/iCloud/둘다/안 할래요) radio + 확인 → signInGoogle·requestApplePermission + users.calendar_preference UPDATE + onClose. 에러 분기(cancelled silent / unauthorized/network 한국어 inline) + busy 다중 호출 차단. §17 anti-AI-feel(brand-500 fill CTA 1개 + surface-2 disabled + 친근체 + 3단계 위계) 정합. `src/lib/calendar/preference.ts` 신규 — fetchCalendarPreference(supabase, userId) helper + Jest 7 tests. `app/group/[id]/index.tsx` wire-up — confirmGroup 성공 분기 NULL preference 감지 → 모달 노출 + 콜백 lazy 구성(expo-* 미설치 환경 페이지 진입 시점 throw 회피). S06 sub-task 10/N 완료
- **다음 update**: S06-ui-reauth-modal(partial_fail_list 감지 + signInGoogleAndUpload 재호출 + 프로필 화면 wire-up) → S06-applesync-hook 전역 wire-up(app/_layout.tsx). 에뮬레이터 시각 검증 + Google OAuth dev key 사용자 prereq 미정
