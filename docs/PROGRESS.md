...
| **Sprint 4** | W3 | S04 + S06 + S12 + S15 (TestFlight) | **S04 ✅ DONE 2026-05-26** (backend + UI via S05-screen-confirm) / **S06 sub-task 9/N 완료 2026-05-26**: queue-foundation + worker-integration + google-oauth + migration-0011 + apple-expo-calendar + worker-google-integration + worker-apple-trigger + **setup** + **applesync-hook** (D34/D35 신규. 양 provider backend + lib + setup wiring + AppState hook 완성. UI 모달 2건만 잔여 — 에뮬레이터 검증 별도 세션) / S12·S15 미시작 | IN_PROGRESS (early start) |
...
| W3 (Sprint 4) | **S04 ✅ DONE 정식** + **S06 sub-task 9/N 완료 2026-05-26**: queue-foundation + worker-integration + google-oauth + migration-0011 + apple-expo-calendar + worker-google-integration + worker-apple-trigger + **setup** + **applesync-hook**. D34(Q-B22 close)·D35 신규 | 4 | Sprint 4 early start. S06 backend + lib + setup wiring + AppState hook 모두 완성. UI 모달 2건 잔여 — 에뮬레이터 시각 검증 별도 세션 |
| W3.5 | — | — | Launch |
...
- **날짜**: 2026-05-26
- **업데이트한 사람**: S06-setup + S06-applesync-hook 연이어 ship. `src/lib/calendar/setup.ts` 신규 — dynamicRequire(expo-auth-session/expo-secure-store/expo-calendar 다음 EAS Build 시점 lazy install) + createGoogleCalendarProvider/createAppleCalendarProvider factory + uploadGoogleTokensToServer(upsert_user_oauth_tokens RPC) + deleteGoogleTokensFromServer + signInGoogleAndUpload combined flow + Jest 10 tests TDD-first. `useApplePendingSync.ts` 신규 — mount + AppState 'active' 전환 시 processApplePendingPushes 호출, concurrent guard(inFlight ref), AppState DI, enabled flag, triggerSync 수동 호출 + Jest 8 tests TDD-first. S06 sub-task 9/N 완료 (backend + lib + setup + hook 모두 완성. UI 모달 2건만 잔여)
- **다음 update**: 다음 세션 — S06-ui-first-time-modal(첫 모달 + users.calendar_preference UPDATE + signInGoogleAndUpload·requestPermission wire-up) → S06-ui-reauth-modal(partial_fail_list 감지 + 재인증). 에뮬레이터 시각 검증 + DESIGN 토큰 정합 + Google OAuth dev key 필요
