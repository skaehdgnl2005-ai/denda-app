...
| **Sprint 4** | W3 | S04 + S06 + S12 + S15 (TestFlight) | **S04 ✅ DONE 2026-05-26** (backend + UI via S05-screen-confirm) / **S06 sub-task 7/N 완료 2026-05-26**: queue-foundation + worker-integration + google-oauth + migration-0011 + apple-expo-calendar + worker-google-integration + **worker-apple-trigger** (D34 implementation — migration 0013 + worker apple_pending INSERT + 클라 applePending lib + Jest 16 tests). D34/D35 신규. setup.ts · UI 모달은 후속 sub-task / S12·S15 미시작 | IN_PROGRESS (early start) |
...
| W3 (Sprint 4) | **S04 ✅ DONE 정식** + **S06 sub-task 7/N 완료 2026-05-26**: queue-foundation + worker-integration + google-oauth + migration-0011 + apple-expo-calendar + worker-google-integration + **worker-apple-trigger** (D34 implementation — migration 0013 + worker apple_pending INSERT + 클라 applePending lib + Jest 16 tests). D34(Q-B22 close)·D35 신규 | 4 | Sprint 4 early start. S06 backend 묶음 + 양 provider 통합 완성. setup.ts · UI 모달 (FirstTimeModal·ReauthModal·hook wire-up) 잔여 — 다음 세션 |
| W3.5 | — | — | Launch |
...
- **날짜**: 2026-05-26
- **업데이트한 사람**: S06-worker-apple-trigger ship — D34 implementation. migration 0013 `calendar_push_apple_pending` table(RLS 본인만 + (group_id,user_id) unique + 미완료 partial index). worker `'apple_ios'`/`'both'` 분기 추가 — `enqueueApplePending` upsert(ON CONFLICT DO NOTHING). `'both'` Promise.all로 Google + Apple 동시. PushContext.currentGroupId 옵셔널 + ctxForGroup closure. 클라 lib `src/lib/calendar/applePending.ts` 신규(`parsePendingRow` 방어적 + `processOneRow` + `processApplePendingPushes` entry) + Jest 16 tests TDD-first. PUSH_FAIL_REASONS.appleEnqueueFailed 추가. S06 sub-task 7/N 완료
- **다음 update**: 다음 세션 — S06-setup (production wiring 어댑터 + token 서버 업로드 wrapper) → S06-ui-first-time-modal → S06-ui-applesync-hook (AppState wire-up) → S06-ui-reauth-modal
