...
| **Sprint 4** | W3 | S04 + S06 + S12 + S15 (TestFlight) | **S04 ✅ DONE 2026-05-26** (backend + UI via S05-screen-confirm) / **S06-queue-foundation ✅ + S06-worker-integration ✅ + S06-google-oauth ✅ + S06-migration-0011 ✅ + S06-apple-expo-calendar ✅ + S06-worker-google-integration ✅ + Q-B22 close (D34) + D35 신규 2026-05-26** (DB schema + pg_cron + 순수 함수 + worker skeleton + Google 클라이언트 lib + users.calendar_preference 컬럼 + Apple expo-calendar 클라이언트 lib + worker Google integration + user_oauth_tokens. apple_pending · setup.ts · UI 모달은 후속 sub-task) / S12·S15 미시작 | IN_PROGRESS (early start) |
...
| W3 (Sprint 4) | **S04-backend ✅** + **S04-UI ✅ via S05-screen-confirm 2026-05-26** → **S04 ✅ DONE 정식** + **S06-queue-foundation ✅ + S06-worker-integration ✅ + S06-google-oauth ✅ + S06-migration-0011 ✅ + S06-apple-expo-calendar ✅ + S06-worker-google-integration ✅ + Q-B22 close (D34) + D35 신규 2026-05-26** (DB schema + pg_cron + 순수 함수 + worker skeleton + Google 클라이언트 lib + users.calendar_preference 컬럼 + Apple expo-calendar 클라이언트 lib + worker Google integration + user_oauth_tokens 서버 측 token storage) | 4 | Sprint 4 early start. Q-B5 close by D33 + Q-B22 close by D34 + D35 신규. S04 acceptance 5/5 완료. S06 sub-task 6/N 완료 — worker pushToMemberCalendar Google 분기 실 구현. apple_pending · setup.ts · UI 모달은 다음 세션 |
| W3.5 | — | — | Launch |
...
- **날짜**: 2026-05-26
- **업데이트한 사람**: S06-worker-google-integration ship — D34 (Apple sync = 클라 polling, Q-B22 close) + D35 (Google token = user_oauth_tokens table) 신규 결정. migration 0012_user_oauth_tokens(RLS + upsert RPC SECURITY DEFINER). `_lib/google_calendar.ts` 신규(buildGoogleEventBody · isAccessTokenExpired · refreshAccessToken · insertCalendarEvent + Deno 23 tests TDD-first). `calendar_push_worker.pushToMemberCalendar` 교체 — users.calendar_preference 분기('google'/'both' → Google API events.insert 실 구현, 'apple_ios'/'none' silent ok). PUSH_FAIL_REASONS 6종 + token rotation 처리. S06 sub-task 6/N 완료
- **다음 update**: 다음 세션 — S06-worker-apple-trigger (migration 0013 apple_pending + worker INSERT + 클라 hook) → S06-setup (production wiring 어댑터 + token 서버 업로드) → S06-ui-first-time-modal → S06-ui-reauth-modal
