...
| **Sprint 4** | W3 | S04 + S06 + S12 + S15 (TestFlight) | **S04 ✅ DONE 2026-05-26** (backend + UI via S05-screen-confirm) / **S06-queue-foundation ✅ 2026-05-26** (DB schema + 순수 함수, worker/OAuth/UI는 후속 sub-task) / S12·S15 미시작 | IN_PROGRESS (early start) |
...
| W3 (Sprint 4) | **S04-backend ✅** + **S04-UI ✅ via S05-screen-confirm 2026-05-26** → **S04 ✅ DONE 정식** + **S06-queue-foundation ✅ 2026-05-26** (DB schema + 순수 함수) | 3 | Sprint 4 early start. Q-B5 close by D33. S04 acceptance 5/5 완료. S06 background queue 기반 (worker · OAuth · UI는 후속 sub-task) |
| W3.5 | — | — | Launch |
...
- **날짜**: 2026-05-26
- **업데이트한 사람**: S06-queue-foundation ship — `groups.calendar_pushed_at` + `calendar_retry_count` 컬럼 + partial index + `_lib/calendar_queue.ts` 순수 함수 3종(`isCalendarPushPending` · `nextRetryState` · `buildCalendarEventPayload`) + Deno 17 tests TDD-first. tsconfig excludes로 RN/jest 영향 0. worker · Google OAuth · expo-calendar · UI 모달은 후속 sub-task
- **다음 update**: S06-worker-integration (`calendar_push_worker` Edge Function + pg_cron 0010 migration) 또는 시간 그리드 투표 → CTA user flow E2E
