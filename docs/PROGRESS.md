...
| **Sprint 4** | W3 | S04 + S06 + S12 + S15 (TestFlight) | **S04 ✅ DONE 2026-05-26** (backend + UI via S05-screen-confirm) / **S06-queue-foundation ✅ + S06-worker-integration ✅ + S06-google-oauth ✅ + S06-migration-0011 ✅ + S06-apple-expo-calendar ✅ + Q-B22 신규 2026-05-26** (DB schema + pg_cron + 순수 함수 + worker skeleton + Google 클라이언트 lib + users.calendar_preference 컬럼 + Apple expo-calendar 클라이언트 lib + Apple sync mechanism Q. worker stub 교체 · setup.ts · UI 모달은 후속 sub-task) / S12·S15 미시작 | IN_PROGRESS (early start) |
...
| W3 (Sprint 4) | **S04-backend ✅** + **S04-UI ✅ via S05-screen-confirm 2026-05-26** → **S04 ✅ DONE 정식** + **S06-queue-foundation ✅ + S06-worker-integration ✅ + S06-google-oauth ✅ + S06-migration-0011 ✅ + S06-apple-expo-calendar ✅ + Q-B22 신규 2026-05-26** (DB schema + pg_cron + 순수 함수 + worker skeleton + Google 클라이언트 lib + users.calendar_preference 컬럼 + Apple expo-calendar 클라이언트 lib + Apple sync mechanism Q) | 4 | Sprint 4 early start. Q-B5 close by D33 + Q-B22 신규(Apple sync mechanism). S04 acceptance 5/5 완료. S06 sub-task 5/N 완료 — worker stub 교체 (서버 측 Google token 저장 D{N} prereq) · setup.ts · UI 모달은 다음 세션 |
| W3.5 | — | — | Launch |
...
- **날짜**: 2026-05-26
- **업데이트한 사람**: S06-apple-expo-calendar ship — `src/lib/calendar/apple.ts` (AppleCalendarProvider DI 패턴 + 순수 함수 2종 buildAppleEvent/pickWritableCalendar + CalendarProviderError reuse from google.ts) + Jest 20 tests TDD-first. Q-B22 신규 추가(Apple sync mechanism — worker → client trigger 패턴, 추천 옵션 (b) 클라 polling). S06 sub-task 5/N 완료. 워커 stub 교체 + setup.ts + UI 모달은 다음 세션
