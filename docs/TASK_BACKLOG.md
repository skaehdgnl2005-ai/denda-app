...
- **Status**: IN_PROGRESS (sub-task 6/N: S06-queue-foundation ✅ + S06-worker-integration ✅ + S06-google-oauth ✅ + S06-migration-0011 ✅ + S06-apple-expo-calendar ✅ + **S06-worker-google-integration ✅** 2026-05-26 — backend 묶음 + 양 provider 클라이언트 lib + users.calendar_preference + Q-B22 close(D34) + D35 신규(user_oauth_tokens) + worker Google API 실 구현. apple_pending(S06-worker-apple-trigger) · setup.ts production wiring · UI 모달 잔여) | **Owner**: Mobile + Backend | **Sprint**: 4 | **Lane**: D
...
- **Acceptance**:
  ...
  - 🟡 `expo-calendar` wrapper (iOS 17+ write-only 권한) — ✅ 클라이언트 lib `src/lib/calendar/apple.ts` (S06-apple-expo-calendar 2026-05-26, AppleCalendarProvider + Jest 20 tests). worker → client trigger는 **S06-worker-apple-trigger** 잔여 (D34 채택 — `calendar_push_apple_pending` table INSERT + 클라 foreground polling)
  - ⏳ "어디 추가할까요" 첫 모달 (Google·Apple·둘 다) — **S06-ui-first-time-modal** (GoogleCalendarProvider.authorize + AppleCalendarProvider.requestPermission + users.calendar_preference UPDATE + Google는 setup.ts wrapper로 `upsert_user_oauth_tokens` RPC 호출)
  - ⏳ Token 만료 → 프로필 "재인증 필요" + 다음 진입 모달 — **S06-ui-reauth-modal** (D19 silent fail 금지, `partial_fail_list.reason='token_expired'`/`'unauthorized'` 감지)
  - 🟡 Partial push fail 시 호스트에게 멤버 list 알림 — ✅ partial_fail_list 누적 backend 완료(S06-worker-integration + S06-worker-google-integration의 PUSH_FAIL_REASONS 6종). 호스트 알림 trigger는 **S06-ui-reauth-modal**과 묶음 또는 별도 push handler에서
  - ✅ Background queue (pg_cron + retry max 3) — **DONE** (S06-queue-foundation + S06-worker-integration)
- **Files**:
  - ✅ ship됨: `supabase/migrations/{0009_calendar_push_queue,0010_calendar_cron,0011_users_calendar_preference,0012_user_oauth_tokens}.sql`, `supabase/functions/_lib/{calendar_queue,google_calendar}.ts(_test.ts)`, `supabase/functions/calendar_push_worker/index.ts`, `src/lib/calendar/{google,apple}.ts(.test.ts)`
  - 잔여: `src/lib/calendar/setup.ts`(production wiring — expo-auth-session + expo-secure-store + expo-calendar 어댑터 + `upsert_user_oauth_tokens` RPC wrapper), `src/components/calendar/{FirstTimeModal,ReauthModal}.tsx`, `supabase/migrations/0013_calendar_push_apple_pending.sql` + worker apple_ios INSERT + 클라 `src/lib/calendar/useApplePendingSync.ts` (D34 implementation)
- **Worktree 분기**: 가능 (Lane D 독립). S06 backend는 `worktree-s06-calendar-sync` branch 위에서 진행 → main 머지 후 cleanup. 본 ship은 `worktree-s06-google-oauth` branch에 4 commits 누적(303471c·67c4dca·bccfe11·worker-google-integration). main 머지 시점은 사용자 결정. 후속 OAuth/UI sub-task는 새 worktree 또는 본 worktree 누적
- **Notes**: backend 묶음 + 양 provider 클라이언트 lib + Google worker 실 구현 ship 완료. 잔여: Apple worker trigger(D34 implementation — apple_pending table + 클라 hook) + setup.ts production wiring + UI 모달. UI 작업은 EAS Build + 디자인 검증 필요로 다음 세션
