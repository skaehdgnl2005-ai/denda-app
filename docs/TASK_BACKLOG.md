...
- **Status**: IN_PROGRESS (sub-task 7/N: S06-queue-foundation ✅ + S06-worker-integration ✅ + S06-google-oauth ✅ + S06-migration-0011 ✅ + S06-apple-expo-calendar ✅ + S06-worker-google-integration ✅ + **S06-worker-apple-trigger ✅** 2026-05-26 — backend 묶음 + 양 provider 클라이언트 lib + worker Google·Apple 분기 모두 실 구현 + apple_pending table + 클라 applePending lib. D34/D35. setup.ts production wiring · UI 모달 · hook wire-up 잔여) | **Owner**: Mobile + Backend | **Sprint**: 4 | **Lane**: D
...
- **Files**:
  - ✅ ship됨: `supabase/migrations/{0009..0013}.sql`, `supabase/functions/_lib/{calendar_queue,google_calendar}.ts(_test.ts)`, `supabase/functions/calendar_push_worker/index.ts`, `src/lib/calendar/{google,apple,applePending}.ts(.test.ts)`
  - 잔여: `src/lib/calendar/setup.ts`(production wiring — expo-auth-session + expo-secure-store + expo-calendar 어댑터 + `upsert_user_oauth_tokens` RPC wrapper), `src/components/calendar/{FirstTimeModal,ReauthModal}.tsx`, `src/lib/calendar/useApplePendingSync.ts`(AppState change + 화면 진입 trigger hook — `processApplePendingPushes` wrapper)
- **Worktree 분기**: 가능 (Lane D 독립). S06 backend는 `worktree-s06-calendar-sync` branch 위에서 진행 → main 머지 후 cleanup. 본 sub-task series는 `worktree-s06-google-oauth` branch에 5 commits 누적(303471c·67c4dca·bccfe11·worker-google-integration·worker-apple-trigger). main 머지 시점은 사용자 결정. 후속 setup·UI sub-task는 새 worktree 또는 본 worktree 누적
- **Notes**: backend 완성 — 양 provider worker 통합 + apple_pending table + 클라 lib. 잔여: setup.ts production wiring + FirstTimeModal/ReauthModal + useApplePendingSync hook. UI 작업은 EAS Build + DESIGN 토큰 적용 검증 필요로 다음 세션
