...
- **총 17 태스크** (S00 ~ S16) + **S05-screen-confirm** sub-task (DONE 2026-05-26) + **S06-queue-foundation** + **S06-worker-integration** + **S06-google-oauth** sub-task (DONE 2026-05-26)
...
- **Status**: IN_PROGRESS (sub-task 3/N: S06-queue-foundation ✅ + S06-worker-integration ✅ + S06-google-oauth ✅ 2026-05-26 — backend 묶음 + Google 클라이언트 lib 완성. Apple expo-calendar · migration 0011 · worker stub 교체 · UI 모달 잔여) | **Owner**: Mobile + Backend | **Sprint**: 4 | **Lane**: D
...
- **Files**:
  - ✅ ship됨: `supabase/migrations/{0009_calendar_push_queue,0010_calendar_cron}.sql`, `supabase/functions/_lib/calendar_queue.ts(_test.ts)`, `supabase/functions/calendar_push_worker/index.ts`, `src/lib/calendar/google.ts(.test.ts)`
  - 잔여: `src/lib/calendar/apple.ts`, `src/lib/calendar/setup.ts` (production wiring — expo-auth-session + expo-secure-store 어댑터), `src/components/calendar/{FirstTimeModal,ReauthModal}.tsx`, `supabase/migrations/0011_users_calendar_preference.sql`
