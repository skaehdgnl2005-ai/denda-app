...
> **베타 구현 범위 (S25 / [D42](DECISIONS.md#d42--홈--나만의-캘린더-prd-51-복귀--수동-개인-일정-활성), 2026-07-28)**
> - **구현됨**: 월간 캘린더 + 선택일 일정 목록 + 다가오는 모임 요약 · 모임(확정/투표 중) · 에브리타임 수업 · 수동 개인 일정 추가/수정/삭제 · "지도로 보기" 진입
> - **이번 범위 제외**: 주간 뷰 토글 · 출처 필터 · **Google/Apple 외부 캘린더 일정 표시**([D19](DECISIONS.md#d19--calendar-sync-단방향-부분-실패-명시) 단방향 push 전용 유지 — 표시하려면 권한 모델부터 재설계 필요)
> - 월 그리드 마커는 모임·개인 일정만. 수업은 매주 반복이라 마커에서 제외하고 선택일 목록에만 표시 (DESIGN §10.6b)
